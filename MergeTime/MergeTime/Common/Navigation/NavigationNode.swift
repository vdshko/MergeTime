...
import class Foundation.NSHashTable
import typealias Foundation.TimeInterval
import struct Foundation.Date
import class Foundation.DispatchQueue

class NavigationNode: NavigationEventDrivenProtocol {
    ...
    public func raise<T: NavigationEvent>(event: T, from sender: NavigationEventDrivenProtocol) {
        // check if we can resolve the event on the same level
        if checkIfCanResolve(event),
           !Configuration.shouldPassEventsOnceHandled {
            return
        }
...
private class EventHandlersContainer<T>: EventHandleable {
    ...
    func propagate(event: T) {
        ...
        if currentEventDescription == lastOpenedEventDescription,
